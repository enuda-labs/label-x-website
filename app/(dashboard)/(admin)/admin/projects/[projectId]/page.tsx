...
const ProjectManagement = () => {
  ...
  const totalAssignedTasks = assignments.reduce(
    (sum, assignment) => sum + assignment.assignedTasks,
    0
  )
  const totalCompletedByLabelers = assignments.reduce(
    (sum, assignment) => sum + assignment.completedTasks,
    0
  )
  const averageAccuracy =
    assignments.length > 0
      ? Math.round(
          assignments.reduce(
            (sum, assignment) => sum + assignment.accuracy,
            0
          ) / assignments.length
        )
      : 0
