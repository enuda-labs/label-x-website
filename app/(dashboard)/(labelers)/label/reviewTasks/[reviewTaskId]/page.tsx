    ...
    try {
      ...
      if (error instanceof AxiosError) {
        if (!error.response) {
          ...
          setError(
            serverMsg ||
              `Failed to load task (status ${error.response.status}).`
          )
        }
      ...
      if (err instanceof AxiosError) {
        ...
        if (!err.response) {
          ...
          switch (status) {
            ...
            case 401:
              message =
                serverMsg || 'Unauthorized. Please sign in and try again.'
              break
            case 403:
              message =
                serverMsg ||
                "Forbidden. You don't have permission to assign this cluster."
              break
            case 404:
              message =
                serverMsg || 'Cluster not found. It may have been removed.'
              break
            case 409:
              message =
                serverMsg || 'Conflict: this cluster may already be assigned.'
              break
            case 422:
              message =
                serverMsg ||
            ...
            case 504:
              message =
                serverMsg || `Server error (${status}). Please try again later.`
              break
            default:
  ...
  return (
    <div className="bg-card/20 min-h-screen">
        ...
        <div className="mb-4 flex items-start gap-2">
          ...
          <div>
            <h1 className="text-xl font-semibold">
              {taskData.title ?? `Task #${taskData.id ?? reviewTaskId}`}
            </h1>
                ...
                <div className="bg-muted/50 rounded-lg p-6">
                  {itemType === 'TEXT' && (
                    <p className="text-lg leading-relaxed">
                      {currentItem?.data || 'No text available'}
                    </p>
                  )}
                  ...
                  {itemType === 'IMAGE' &&
                    (currentItem?.file_url ? (
                      ...
                      <p className="text-sm text-gray-500">
                        <AlertCircle className="mr-1 inline-block" />
                        No image available
                      </p>
                    ))}
              ...
              <CardContent className="flex flex-wrap gap-2">
                {choicesToShow.map((choice, index) => (
                  <Badge
                    key={index}
                    variant="secondary"
                    className="cursor-default px-3 py-1 text-sm opacity-80 select-none"
                  >
                    {choice.option_text}
                  </Badge>
                ))}
      ...
      <Dialog open={confirmAssignOpen} onOpenChange={setConfirmAssignOpen}>
        <DialogContent className="flex max-w-lg flex-col items-center justify-center sm:top-1/2 sm:-translate-y-1/2">
          <DialogHeader>
            <DialogTitle>Confirm Assignment</DialogTitle>
            <DialogDescription>
              Are you sure you want to assign this cluster (
              <b>{taskData.labeller_instructions}</b> #{taskData.id}) to
              yourself?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
