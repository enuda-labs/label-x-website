...
export default function ApiKeys() {
  ...
  const revokeApiKey = async (keyId: string) => {
    try {
      ...
      setApiKeys(prev => prev.map(key => (key.id === keyId ? { ...key, revoked: true } : key)));
    ...
    <DashboardLayout title="API Keys">
      <Alert className="mb-8 bg-white/5 border-white/10">
        ...
        <AlertDescription className="text-white/70">
          API keys allow you to authenticate requests to Label X services. Keep your keys secure and
      ...
      {generatedKey && (
        <Card className="bg-white/5 border-white/10 mb-8">
          <div className="p-6 border-b border-white/10">
            <h2 className="text-xl font-semibold text-white">Newly Generated Key</h2>
          ...
          <Table>
            ...
            <TableBody>
              <TableRow className="border-white/10 hover:bg-white/5">
                <TableCell className="text-white font-medium">{generatedKey.name}</TableCell>
