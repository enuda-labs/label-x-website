      ...
      <div className="mx-auto max-w-5xl overflow-hidden rounded-lg border border-slate-100 shadow-sm">
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="overflow-hidden rounded-md border border-slate-100">
            <div className="max-h-96 divide-y divide-slate-100 overflow-auto">
              {segments.map((seg, idx) => (
                <div
                  key={seg.id}
                  className={`flex items-start gap-4 p-4 ${previewIndexRef.current === idx ? 'bg-slate-50' : 'bg-white'}`}
                >
                  <div className="w-40 text-xs text-slate-600">
                    <div className="font-mono text-xs text-slate-700">
                      {formatHHMMSSms(seg.start)} → {formatHHMMSSms(seg.end)}
                    </div>
                  ...
                  <div className="flex-1">
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="ml-auto rounded border border-transparent px-2 py-1 text-sm text-rose-600 hover:bg-rose-50"
                      >
                        Delete
                      </button>
                    </div>
                    ...
                    <textarea
                      ...
                      className="min-h-[56px] w-full resize-none rounded border border-slate-100 p-3 text-sm"
                    />
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      />
                      <button
                        className="rounded border border-slate-200 bg-white px-3 py-1 text-sm"
                        onClick={() =>
                          updateSegment(seg.id, {
          ...
          <div className="text-xs text-slate-400">
            Tip: Keep each segment short (5–10s) for best readability. This UI
            is tuned for Igbo-language transcribers with minimal distractions.
          </div>
        </div>
      </div>
    </div>
