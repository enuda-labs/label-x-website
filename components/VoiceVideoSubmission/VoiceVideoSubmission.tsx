...
import SubtitleAnnotator, {
  generateSRTBlob,
} from '@/components/SubtitleAnnotator/SubtitleAnnotator'
    ...
    try {
      ...
      if (subtitleSegments && subtitleSegments.length > 0) {
        try {
          ...
          console.warn(
            'Subtitle upload failed, continuing without subtitles:',
            srtErr
          )
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (mr.state === 'recording') {
          ...
          console.log(
            `Recording stopped automatically after ${MAX_RECORDING_MINUTES} minutes.`
          )
        }
      }, MAX_RECORDING_MS)
    } catch (err) {
