...
export const TaskCard = ({ task }: { task: TaskItem }) => {
  const progressPercentage = (task.completedItems / task.labeller_per_item_count) * 100;
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <div className={`flex items-center gap-1 text-xs ${getStatusColor(task.status)}`}>
                  {getStatusIcon(task.status)}
                  <span className="capitalize">{task.status}</span>
                </div>
                {/*  eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                <Badge variant={getPriorityColor(task.priority) as any} className="text-xs uppercase font-medium">
                  {task.priority}
                </Badge>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <Link href={`/label/${task.id}`}>
            <Button 
              ...
              size="sm"
            >
              {task.status === 'completed' ? 'View Results' : 'Continue Labeling'}
